...
public class GradeViewModel extends ViewModel {
    ...
    private MutableLiveData<List<Grade>> models;

    public void init(String url) {
        setLessons(url);
    }

    public MutableLiveData<List<Grade>> getModels() {
        if (models == null) {
            MutableLiveData<List<Grade>> liveData = new MutableLiveData<>();
            liveData.setValue(new ArrayList<Grade>());
            models = liveData;
        }
        return models;
    }
                ...
                .getAsJSONObject(new JSONObjectRequestListener() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            Log.e("error", "grades are here!");

                            Type listType = new TypeToken<ArrayList<Grade>>() {
                            }.getType();
                            List<Grade> newLessons = new Gson().fromJson(response.getJSONArray("data").toString(), listType);

                            List<Grade> currentLessons = models.getValue();
